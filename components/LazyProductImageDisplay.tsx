...
export function LazyProductImageDisplay({ imgs, img, alt }: LazyProductImageDisplayProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full relative group/img overflow-hidden flex items-center justify-center bg-white rounded"
    >
      {!isLoaded && isInView && (
      ...
      {isInView && (
        <>
          <Image
            ...
            className={`object-contain mix-blend-multiply transition-all duration-500 ease-in-out group-hover/img:scale-105 ${
          ...
          {secondaryImg && (
            <Image
              ...
              fill
              sizes="(max-width: 768px) 50vw, 25vw"
              className="absolute inset-0 object-contain mix-blend-multiply opacity-0 transition-all duration-500 ease-in-out group-hover/img:opacity-100 group-hover/img:scale-105"
            />
          )}
        </>
      )}
