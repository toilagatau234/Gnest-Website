...
import type { HeroContent } from '@/lib/services/site-content';
...
export function HeroSection({ content }: HeroSectionProps) {
  ...
  const title = content?.title || 'SẢN XUẤT THEO YÊU CẦU';
  const subtitle = content?.subtitle || 'Đa dạng mẫu mã · Chất lượng đảm bảo · Giao hàng toàn quốc';
  const ctaText = content?.cta_text || 'Liên hệ ngay';
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-dtl-navy-dark to-[#1a3060] text-white py-12 pb-13">
      ...
      />
      <div className="absolute -top-[60px] -right-[60px] w-[520px] h-[520px] rounded-full bg-[radial-gradient(circle,rgba(227,30,36,0.18)_0%,transparent_70%)] pointer-events-none" />
      ...
      <div className="max-w-[1220px] mx-auto px-5 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className="text-[26px] md:text-4xl lg:text-[40px] font-black leading-[1.12] mb-2 tracking-tight whitespace-pre-line">
              {title}
            </h1>
          ...
          <div className="hidden md:flex flex-col gap-3.5 relative">
            ...
            <div className="flex gap-2.5 justify-center mt-2 relative z-20">
              {['Giá sỉ tốt', 'Ship toàn quốc', 'In theo yêu cầu'].map((badge) => (
                <div key={badge} className="bg-white/10 border border-white/20 rounded-full py-1.5 px-4 text-xs font-semibold text-white/85 flex items-center gap-1.5">
