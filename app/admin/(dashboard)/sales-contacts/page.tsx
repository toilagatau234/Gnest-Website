    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredContacts.map((contact) => (
          <div key={contact.id} className="bg-white hover:border-slate-300 border border-[#E2E8F0] rounded-2xl p-5 shadow-xs relative flex flex-col justify-between group transition-all hover:shadow-md">
            
            {/* Options absolute header button */}
            <button 
              onClick={() => alert('Chi tiết chỉnh sửa nhân sự đang ở chế độ visual mockup.')}
              className="absolute top-4 right-4 p-1.5 bg-white text-slate-500 hover:text-slate-800 rounded-lg shadow-xs border border-slate-200 hover:border-slate-350 transition-all opacity-80 group-hover:opacity-100"
        ...
        {filteredContacts.length === 0 && (
          <div className="col-span-full py-16 text-center border-2 border-dashed border-slate-250 rounded-2xl bg-slate-50/50">
